...
void convert(FILE* input, FILE* executable)
{
  ...
  while (fgets(line, LINE_LEN + 1, input) != NULL)
  {
    ...
    else if ( sscanf(line, "%s%lf%lf%lf%lf", command, &read_1, &read_2,
		     &read_3, &read_4) == 5 &&
              strncmp(command, "DRAWSegment", DRAW_LEN + 1) == 0
	    )
    {
      sent_1 = lround(read_1);
      sent_2 = lround(read_2);
      sent_3 = lround(read_3);
      sent_4 = lround(read_4);
