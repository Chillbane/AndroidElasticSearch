...
// could be called for other siganls, so signal aprameter indicates which one
void handle_timeout(int signal)
{
  ...
  if (signal == SIGALRM) {
    ...
    // stops timer on 5th call, should do this when game is done
    if (called == 5) {
...
// Protect critical code (e.g. moving the ship) by blocking the signal
void block_signal()
{
  // remember old list of signals to block (i.e. none)
  sigset_t old_mask;

  // set blocked signal set to block_mask_g (so blocks SIGALRM)
  if (sigprocmask(SIG_BLOCK, &block_mask_g, &old_mask) < 0)
    exit(EXIT_FAILURE);

  // CRITICAL CODE

  // unblock signal by setting mask to old value
