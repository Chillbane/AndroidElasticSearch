...
void handle_timeout(int signal)
{
  ...
  if (signal == SIGALRM) {
    ...
    if (game_done == TRUE) {
      ...
      /* check that ship is upright and it is descending sufficiently slowly */
      if ((thrust_angle == PI/2) && (y_velocity <= 20)) {

	/* check that the piece of land the ship contacted is flat */
	if (landscape.y_coordinates[land_i] == 
	    landscape.y_coordinates[land_i+1]) {
	  printw("LANDED!!!");
	}
	else {
	  printw("NOT PARALLEL CRASHED!!!, %Lf", y_velocity);
	}
      }
